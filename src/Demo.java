...
public class Demo {
	 ...
	 * 
	 */
	public static void runTrainAndTest(String dataPath, String modelDir, double d2) {
		...
		// 	+ "-train /sharenvme/usershome/hyx/data/THUCNews/THUCNews "  // 设置您的训练路径，这里的路径只是给出样例
		// 	+ "-test /sharenvme/usershome/hyx/data/THUCNews/THUCNews " 。
		// 	+ "-d1 0.7 "  // 前70%用于训练
		// 	+ "-d2 0.3 "  // 后30%用于测试
		// 	+ "-f 5000 " // 设置保留特征数，可以自行调节以优化性能
		// 	+ "-s /sharenvme/usershome/hyx/THUCTC/my_novel_model"  // 将训练好的模型保存，便于以后测试或部署时直接读取模型，无需训练
		// 	;
	...
	public static void main (String args[]) {
		if (args.length < 3) {
			System.err.println("Usage: <data path> <modelDir> <d2>");
			System.exit(1);
		}
        ...
        try {
            ...
            System.err.println("The third parameter must be between [0, 1], for example 0.3");
            System.exit(1);
            return;
        }

		runTrainAndTest(dataPath, modelDir, d2);
		// runLoadModelAndUse();
